...
# АН Эва — AI-консультант RIZALTA v1.3.0

📅 **Последнее обновление:** 19.02.2026 (сессия 6)
...
- OpenAI Responses API: **gpt-4o-mini** (extractor, analyzer), **gpt-5.2** (generator, reasoning отключён)
...
### Скорость: ~5.6 сек до первого токена (было ~11)
Три последовательных LLM-вызова (параллелизация невозможна — analyzer зависит от extractor).
Оптимизировано в сессии 6: reasoning убран из generator, extractor prompt сжат 3.5x (2000→584 токенов), signals убраны (мёртвый груз), оставлен urgency.
Можно ещё: gpt-4o-mini для generator, кеширование RAG.

---

## 🔜 Задачи (приоритет)

### P1 — ДДУ и договор управления (СЛЕДУЮЩАЯ СЕССИЯ):

**1. Изучить ДДУ и договор аренды/управления**
Файлы: `/opt/bot/docs/ddu.pdf` (11 стр), `/opt/bot/docs/arenda.pdf` (54 стр)
ВАЖНО: ДДУ оформлен как квартира (квартирник), НЕ апартаменты!
Извлечь ключевые позиции, обновить knowledge base и промпт Марго.

**2. Обновить rizalta_context.py и промпт**
Марго должна корректно отвечать на вопросы по ДДУ, условиям договора с УК, МГП.

**3. Протестировать воронку вживую**
Прогнать 3-5 сценариев: инвестор, для себя, возражения, отказ.
Проверить что МГП-крючок и 2 вопроса работают корректно.
...
### Сессия 6 (19.02.2026):
- Greeting: Марго + ДДУ/УК + цена-фильтр 14.3 млн + окупаемость 6.5 лет
- Промпт: 2 вопроса (бюджет + сроки) вместо 3, МГП-крючок (факт без цифр)
- Quick replies: «Какие есть варианты?», «Рассчитать доход», «Как работает гарантия дохода?»
- Urgency в state_summary для генератора
- Скорость 18→9.4 сек: reasoning убран, extractor prompt 3.5x compact, signals→flat urgency
- Найдены ДДУ и договор аренды: /opt/bot/docs/ddu.pdf (11 стр), /opt/bot/docs/arenda.pdf (54 стр)
- ВАЖНО: ДДУ оформлен как квартира (квартирник), НЕ апартаменты — изучение в следующей сессии
